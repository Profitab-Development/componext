...
//             </div>
//
//             {/* Navigation Arrows — DESKTOP (як було) */}
...
//             </div>
//
//             {/* Navigation Arrows — MOBILE (по центру, 56×56, ближче до контенту) */}
...
export const Applications = memo(() => {
  ...
  // Синхронізація скролу зі зміною currentSlide
  useEffect(() => {
    if (sliderRef.current) {
      const containerWidth = sliderRef.current.clientWidth
      sliderRef.current.scrollTo({
  ...
  // Визначення слайду після свайпу
  useEffect(() => {
    const container = sliderRef.current
    if (!container) return
              ...
              {slides.map((slide, index) => (
                  ...
                  >
                    ...
                    <div className="block lg:hidden rounded-2xl overflow-hidden bg-white px-4 pt-8 pb-4">
                      ...
                      {/* Image — 100% ширини, але з max-height щоб стрілки не «тікали» вниз */}
                      <div className="relative w-full max-h-[360px] mx-auto flex items-center justify-center">
                        <Image
                            src={slide.backgroundMobile}
                            alt={slide.title}
                            width={800}
